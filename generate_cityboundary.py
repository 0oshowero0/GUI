...
all_city_gpd = gpd.GeoDataFrame({'CITY_NAME': all_city_city_name_list, 'ADMIN_NAME': all_city_province_name_list, 
...
gdp = gdp.merge(country_info, on='CountryCode', how='left')
...
all_gdp = gdp.merge(percapitagdp, on=['CountryCode','CountryName','Region','IncomeGroup'], how='left')


# 世界银行采用3位国家编码，而Esri WorldCity采用2位国家编码，需要转换
country_code_lut = pd.read_csv('country_code_lut.csv')[['Alpha2','Alpha3']]
#all_city_gpd = selected_city
all_city_gpd = all_city_gpd.merge(country_code_lut, left_on='CountryCode', right_on='Alpha2', how='left')

# 与gdp数据合并
all_city_gpd = all_city_gpd.drop(['CountryCode', 'CountryName'], axis=1).merge(all_gdp, left_on='Alpha3', right_on='CountryCode', how='left')
all_city_gpd = all_city_gpd.drop(['Alpha2','area'], axis=1).rename(columns={'CITY_NAME':'CityName', 'ADMIN_NAME':'StateName'})
...
# all_city_gpd.to_file('all_city_boundary.geojson', driver='GeoJSON')  # 可用QGIS打开并选择合适的边界
# all_city_gpd.to_parquet('all_city_boundary.parquet') # parquet格式适合大量数据，可以快速读取
